...
public class Utente {
    ...
    public void restituisci(int id) {
        ...
        for (Risorsa r : libriPresiInPrestito) { // Scorro la lista delle risorse prese in prestito
            if (r.getCodiceIdentificato() == id) { // Controllo per id
                risorsa = r; // Se trovato assegno la risorsa alla variabile
                break;
            }
        }
        if (libriPresiInPrestito.contains(risorsa)) { // Controllo se nella libriPresiInPrestito è presente la risorsa
            // passata
    ...
    public void prendiInPrestito(ArrayList<Risorsa> catalogo, int id) {
        for (Risorsa r : catalogo) {
            if (r.getCodiceIdentificato() == id) { // Controllo per id
                if (r.isDisponibile()) {
                    r.setDisponibile(false); // se corrispondesetto la disponibilita su false perche il libro non è piu
                                             // disponibile
